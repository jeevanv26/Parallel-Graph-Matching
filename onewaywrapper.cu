...
int one_way_handshake(GraphData graph, int *& matches, int numthreads, bool extra_credit)
{
	...
	cudaMalloc((void**) &temp3_gpu,numEdges * sizeof(int));
	cudaMalloc((void**) &temp4_gpu,numEdges * sizeof(int));
    ...
    for (iter = 0; ; iter++) {
		...
		if(extra_credit) {
			...
			for(int x = 0; x < numVertices; x++) {
				strongNeighbor_cpu[x] = UNMATCHED;
			}
			for(int x = 0; x < numEdges; x++) {
				...
				if(strongNeighbor_cpu[src] == UNMATCHED || strongNeighborWeight_cpu[src] < wgt) {
